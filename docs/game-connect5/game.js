...
const badMoveSnd = new sound("game-connect5/audio/BadMove.mp3"); // Sound when not the players turn
const gameOverSnd = new sound("game-connect5/audio/Win.mp3"); // Sound when game won
...
function resetGameButton() {
	...
	if (game.currentPlayer != 0) {
		socket.emit('resetGame', { player: player.name, gameID: game.gameID, winner: (game.winner==0)?PLAYER_1:game.winner });
		resetGame();
	}
}
